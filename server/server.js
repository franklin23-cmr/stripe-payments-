...
// 'endpoins pou gerer le qrcode
// ' 

// liste de tous les produits
app.get('/get-products', async (req, res) => {
  // dans notre cas on va utilise celui de stripe pour une demo,
  // vous pouvez utilisez votre propre bd
...
// creer une checkout session 
app.post('/create-checkout-session', async (req, res) => {
  console.log("create-checkout-session")
  const LocalNetwork = 'http://192.168.254.221:3000'
  ...
  const line_items = data.map((item) => {
    return {
      price_data: {
        ...
        unit_amount: parseInt (item.prix * item.quantite),
      },
      quantity: item.quantite,
      adjustable_quantity: adjustableQuantitySetting
  ...
  const {id: session} = await stripe.checkout.sessions.create({
  success_url:  `${LocalNetwork}/success?session_id={CHECKOUT_SESSION_ID} `,
  cancel_url:   `${LocalNetwork}/cancel`,
...
app.get('/config-session', async (req, res) => {
    res.json({
      publishableKey: process.env.STRIPE_PUBLISHABLE_KEY,
    })
